...
impl Archetype {
	...
	pub fn add_component_type(&mut self, id: ComponentId, m: NonNull<u8>) {
...
impl Archetypes {
	...
	pub fn get_id_or_insert_by_ident(&mut self, type_id: TypeId, components: Vec<ComponentId>, component_infos: &Vec<ComponentInfo>, containers: Vec<NonNull<u8>>) -> ArchetypeId {
		...
		*self
			...
			.or_insert_with(move || {
				...
				for c in containers.into_iter() {
					archetype.add_component_type(components[i].clone(), c);
