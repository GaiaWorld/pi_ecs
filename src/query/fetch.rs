...
impl<'w, T: Component> Fetch<'w> for ReadFetch<T> {
    ...
    unsafe fn archetype_fetch(&mut self, archetype_index: usize) -> Option<Self::Item> {
        match self.storage_type {
            StorageType::Table => {
				unsafe {std::mem::transmute((&mut *(self.container.assume_init_mut().as_ptr() as usize as *mut SecondaryMap<Local, T>)).get(Local::new(archetype_index))) }
            }
            StorageType::SparseSet => {
                unsafe {std::mem::transmute((&mut *(self.container.assume_init_mut().as_ptr() as usize as *mut SparseSecondaryMap<Local, T>)).get(Local::new(archetype_index)))}
            }
        }
    }
}
