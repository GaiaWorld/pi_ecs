...
/// 每个事件都可以被多个系统 并行的 消耗；
///
/// 需要显示指定 读写之间的 顺序；否则会引起 竞争条件；下一次 [`Events::update`] 到达 之前或之后是无法预测的；
///
...
/// # Example
/// ```
/// use pi_ecs::sys::param::event::Events;
///
/// struct MyEvent {
///     value: usize
/// }
...
/// - [`EventReader`] 在 两次更新 中 读取一次 可能仍会收到一些事件
/// - [`EventReader`] 在 两次更新 后 读取的内容，保证会 删除 在 这些更新 之前 发生的 所有事件
///
/// 如 没有调用 [`update`](Events::update)，则 buffers in [`Events`] 会不断的增长；
///
...
#[doc(hidden)]
pub struct EventReaderState<TSystemParamState, T> {
    state: TSystemParamState,
    marker: std::marker::PhantomData<fn() -> T>,
}

// SAFE: only local state is accessed
unsafe impl<TSystemParamState: SystemParamState, T: Component> SystemParamState
    for EventReaderState<TSystemParamState, T>
{
    type Config = ();
...
#[doc(hidden)]
pub struct EventWriterState<TSystemParamState, T> {
    state: TSystemParamState,
    marker: std::marker::PhantomData<fn() -> T>,
}

unsafe impl<TSystemParamState: SystemParamState, T: Component> SystemParamState
    for EventWriterState<TSystemParamState, T>
{
    type Config = ();
    ...
    for EventWriterState<(<ResMut<Events<T>> as SystemParam>::Fetch,), T>
{
    ...
    unsafe fn get_param(
        ...
        Self::Item {
            events: <<ResMut<Events<T>> as SystemParam>::Fetch as SystemParamFetch>::get_param(
                &mut state.state.0,
                system_state,
                world,
                change_tick,
...
mod tests {
    ...
    fn get_events<T: Component + Clone>(
        events: &Events<T>,
